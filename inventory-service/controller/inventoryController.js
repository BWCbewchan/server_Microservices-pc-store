...
exports.syncInventory = async (req, res) => {
    try {
        ...
        if (!products || products.length === 0) {
            return res.status(404).json({ message: "Không có dữ liệu sản phẩm để đồng bộ" });
        }
        ...
        const syncResults = await Promise.all(
            products.map(async (product) => {
                try {
                    ...
                    console.error(`🚨 Lỗi cập nhật sản phẩm ${product._id}:`, updateError);
                    return null;
                }
            })
        );

        const successfulUpdates = syncResults.filter(result => result !== null);
        console.log(`✅ Đồng bộ thành công ${successfulUpdates.length}/${products.length} sản phẩm`);
        
        res.json({
            success: true,
            updated: successfulUpdates.length,
            failed: products.length - successfulUpdates.length
        });
    } catch (error) {
...
// ------------------------------
// 8️⃣ API xác nhận đơn hàng (Improved version)
// ------------------------------
exports.confirmOrder = async (req, res) => {
    try {
        // Parse from body instead of params
        ...
        const results = await Promise.all(
            items.map(async (item) => {
                try {
                    ...
                    await axios.put(`${PRODUCT_UPDATE_STOCK_URL}/${item.productId}`, {
                        quantity: updated.quantity
                    }, { timeout: 5000 });

                    return { success: true, productId: item.productId };
                } catch (error) {
                    console.error(`🚨 Lỗi xác nhận đơn hàng cho ${item.productId}:`, error.message);
                    return { 
                        success: false, 
                        productId: item.productId,
                        error: error.message 
        ...
        if (failedItems.length > 0) {
            return res.status(207).json({ // 207 Multi-Status
                success: false,
                message: "Một số sản phẩm xác nhận không thành công",
                failedItems
            });
        }
